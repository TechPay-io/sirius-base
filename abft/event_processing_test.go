...
func testSiriusRandom(t *testing.T, weights []pos.Weight, cheatersCount int) {
	testSiriusRandomAndReset(t, weights, false, cheatersCount, false)
	testSiriusRandomAndReset(t, weights, false, cheatersCount, true)
	testSiriusRandomAndReset(t, weights, true, 0, false)
	testSiriusRandomAndReset(t, weights, true, 0, true)
}

// TestSirius 's possibility to get consensus in general on any event order.
func testSiriusRandomAndReset(t *testing.T, weights []pos.Weight, mutateWeights bool, cheatersCount int, reset bool) {
